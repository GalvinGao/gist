...
while True:
    ...
    if len(recentHistory) >= 5:
        ...
        recentHistory = []
    if sensorData < 10:
        print('| -- [ Near to the Barrier (distance=%icm); Stopping Motors... ]' % sensorData)
        ...
        break
    time.sleep(0.2)  # Loop too fast might cause program to lag and may drain your battery.
...
motorLeft.run_to_rel_pos(position_sp=shouldMoveDistance, speed_sp=motorSpeedConstant, stop_action='hold')
motorRight.run_to_rel_pos(position_sp=shouldMoveDistance, speed_sp=motorSpeedConstant, stop_action='hold')
